...
window.addEventListener("load", function () {
    ...
    const gendata = {
        ...
        singleswap: (n) => {
            ...
            let i = Math.floor(Math.random() * n / 2), j = Math.floor(n / 2 + Math.random() * n / 2);
        ...
        selection: async (arr) => {
            for (let i = 0; i < arr.length - 1; i++) {
                ...
                for (let j = i + 1; j < arr.length; j++) {
                    if (await islessthan(arr, j, minimum))
                        minimum = j;
                }
                if (minimum != i)
                    await swap(arr, i, minimum);
            }
        },
        quick: async (arr) => {
    ...
    sim.render = function () {
        ...
        sim.withCanvasState(() => {
            ...
            state.data.map(h => h * sim.canvas.height * 0.9).forEach((h, i) => {
                ...
                if (state.hiswap.includes(i))
                    barcolor = "#ff0000";
                else if (state.hicompare.includes(i))
                    barcolor = sim.colours.accent;
                sim.withCanvasState(() => {
    ...
    new ComboBox("method", "Sorting Method", value => {
        if (!value)
            return;
        params.method = value;
    })
    ...
    const startbutton = new Button("start", "Sort", () => {
        if (state.sorted)
            state.data = params.datagen(params.datalength);
