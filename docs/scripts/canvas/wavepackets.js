...
window.addEventListener("load", function () {
    ...
    const selectCombo = new ComboBox("select", "Selected Wave", index => {
        ...
        for (const prop in sliders) {
            ...
            if (sliders.hasOwnProperty(prop))
                sliders[prop].setValue(state.selected[prop]);
        }
    });
    ...
    const getDisplacement = (wave, x, t) => wave.amp * Math.cos(getAngFreq(wave) * t + (wave.reverse ? 1 : -1) * getWaveNum(wave) * x + wave.offset);
