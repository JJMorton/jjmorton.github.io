...
var _HexagonGrid_instances, _HexagonGrid_axialHash;
...
class HexagonGrid {
    constructor(radius) {
        _HexagonGrid_instances.add(this);
        this.radius = radius;
        this.cells = new Map();
    }
    init(initFunc) {
    ...
    getCell(q, r) {
        const hash = __classPrivateFieldGet(this, _HexagonGrid_instances, "m", _HexagonGrid_axialHash).call(this, q, r);
    ...
    hasCell(q, r) {
        return this.cells.has(__classPrivateFieldGet(this, _HexagonGrid_instances, "m", _HexagonGrid_axialHash).call(this, q, r));
    }
    neighboursOf(q, r) {
    ...
    *[(_HexagonGrid_instances = new WeakSet(), _HexagonGrid_axialHash = function _HexagonGrid_axialHash(q, r) {
...
window.addEventListener("load", async function () {
    ...
    function playPause() {
        if (playPauseButton)
            playPauseButton.DOM.removeEventListener("click", playPause);
        if (sim.timer.isPaused) {
            sim.timer.start();
    ...
    sim.render = function () {
        ...
        if (sim.timer.isPaused && !oneRender)
            return;
        // @ts-ignore
        ...
        programWrapper
            ...
            .setUniform("accentcolor", new Float32Array(hexToRGB(sim.colours.accent).map(x => x / 255)))
