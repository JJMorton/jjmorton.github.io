...
window.addEventListener("load", function () {
    ...
    let integrator = new Integrators.RK4Integrator(acceleration, [0.8 * Math.PI, 0.9 * Math.PI], [0, 0], 0.01);
    ...
    new ComboBox("method", "Integration method", method => {
        if (method === null)
            return;
        integrator = new (method)(integrator.funcAccel, integrator.pos, integrator.vel, integrator.h);
