...
var _Rod_l, _Rod_r, _Rod_a, _Reuleaux_r, _Reuleaux_R, _Reuleaux_h, _Reuleaux_phi, _Tutorial_instances, _Tutorial_hint, _Tutorial_stepCompleteCallback, _Tutorial_repositionHint, _Tutorial_loadCurrentStep, _Tutorial_unloadCurrentStep;
...
class Rod extends ParametricShape {
    constructor(length, aspectratio) {
        super();
        _Rod_l.set(this, 0); // Half-length of the rod
        _Rod_r.set(this, 0); // Radius of ends
        _Rod_a.set(this, 1); // Aspect ratio (width / length)
        __classPrivateFieldSet(this, _Rod_a, aspectratio, "f");
        __classPrivateFieldSet(this, _Rod_l, length * (1 - __classPrivateFieldGet(this, _Rod_a, "f")), "f");
        __classPrivateFieldSet(this, _Rod_r, __classPrivateFieldGet(this, _Rod_a, "f") * length, "f");
    }
    set length(val) {
        // When we set the length, scale up the entire rod, mantaining aspect ratio
        const gamma = val / this.length;
        __classPrivateFieldSet(this, _Rod_l, __classPrivateFieldGet(this, _Rod_l, "f") * gamma, "f");
        __classPrivateFieldSet(this, _Rod_r, __classPrivateFieldGet(this, _Rod_r, "f") * gamma, "f");
    }
    get length() {
        // Length is from tip to tip, i.e. the length of the straight section plus the radii of the ends
        return 2 * (__classPrivateFieldGet(this, _Rod_l, "f") + __classPrivateFieldGet(this, _Rod_r, "f"));
    }
    set aspectratio(val) {
        // When we set aspect ratio, keep the length the same and adjust width
        const length = this.length;
        __classPrivateFieldSet(this, _Rod_a, val, "f");
        this.length = length;
    }
    get aspectratio() {
        return __classPrivateFieldGet(this, _Rod_a, "f");
    }
    get perimeter() {
        return 2 * Math.PI * __classPrivateFieldGet(this, _Rod_r, "f") + 4 * __classPrivateFieldGet(this, _Rod_l, "f");
    }
    get minRadius() {
        return __classPrivateFieldGet(this, _Rod_r, "f");
    }
    get maxRadius() {
        return Infinity;
    }
    parametric(s) {
        ...
        s -= __classPrivateFieldGet(this, _Rod_l, "f");
        if (s < 0)
            s += this.perimeter;
        // Curve on RHS
        if (s < Math.PI * __classPrivateFieldGet(this, _Rod_r, "f")) {
            return new Vector([-__classPrivateFieldGet(this, _Rod_r, "f") * Math.sin(s / __classPrivateFieldGet(this, _Rod_r, "f")) - __classPrivateFieldGet(this, _Rod_l, "f"), __classPrivateFieldGet(this, _Rod_r, "f") * Math.cos(s / __classPrivateFieldGet(this, _Rod_r, "f"))]);
        }
        // Straight edge at bottom
        else if (s < Math.PI * __classPrivateFieldGet(this, _Rod_r, "f") + 2 * __classPrivateFieldGet(this, _Rod_l, "f")) {
            return new Vector([-__classPrivateFieldGet(this, _Rod_l, "f") + s - Math.PI * __classPrivateFieldGet(this, _Rod_r, "f"), -__classPrivateFieldGet(this, _Rod_r, "f")]);
        }
        // Curve on LHS
        else if (s < 2 * Math.PI * __classPrivateFieldGet(this, _Rod_r, "f") + 2 * __classPrivateFieldGet(this, _Rod_l, "f")) {
            return new Vector([-__classPrivateFieldGet(this, _Rod_r, "f") * Math.sin((s - 2 * __classPrivateFieldGet(this, _Rod_l, "f")) / __classPrivateFieldGet(this, _Rod_r, "f")) + __classPrivateFieldGet(this, _Rod_l, "f"), __classPrivateFieldGet(this, _Rod_r, "f") * Math.cos((s - 2 * __classPrivateFieldGet(this, _Rod_l, "f")) / __classPrivateFieldGet(this, _Rod_r, "f"))]);
        }
        // Straight edge at top
        else {
            return new Vector([3 * __classPrivateFieldGet(this, _Rod_l, "f") - s + 2 * Math.PI * __classPrivateFieldGet(this, _Rod_r, "f"), __classPrivateFieldGet(this, _Rod_r, "f")]);
        }
    }
    penPoints() {
...
_Rod_l = new WeakMap(), _Rod_r = new WeakMap(), _Rod_a = new WeakMap();
Rod.shape = "rod";
class Reuleaux extends ParametricShape {
    constructor(radius, roundness) {
        super();
        _Reuleaux_r.set(this, 0); // Radius of circle enclosing equilateral triangle
        _Reuleaux_R.set(this, 0); // Radius of corners
        _Reuleaux_h.set(this, 0); // Height below the top tip of the triangle that the centre of the corner circle is
        _Reuleaux_phi.set(this, 0); // 1/2 the angle that the rounded corners cover
        __classPrivateFieldSet(this, _Reuleaux_r, radius, "f");
        __classPrivateFieldSet(this, _Reuleaux_R, roundness * (Math.sqrt(3) - 1) * this.r, "f");
        this.calcProperties();
    }
    set r(r) {
        __classPrivateFieldSet(this, _Reuleaux_r, r, "f");
        this.calcProperties();
    }
    get r() {
        return __classPrivateFieldGet(this, _Reuleaux_r, "f");
    }
    set roundness(val) {
        __classPrivateFieldSet(this, _Reuleaux_R, val * (Math.sqrt(3) - 1) * this.r, "f");
        this.calcProperties();
    }
    get roundness() {
        return __classPrivateFieldGet(this, _Reuleaux_R, "f") / ((Math.sqrt(3) - 1) * this.r);
    }
    get perimeter() {
        const sideLength = Math.sqrt(3) * this.r * (2 * Math.PI / 3 - 2 * __classPrivateFieldGet(this, _Reuleaux_phi, "f"));
        const cornerLength = __classPrivateFieldGet(this, _Reuleaux_R, "f") * 2 * __classPrivateFieldGet(this, _Reuleaux_phi, "f");
    ...
    get maxRadius() {
        return this.roundness === 1 ? __classPrivateFieldGet(this, _Reuleaux_R, "f") : Math.sqrt(3) * this.r;
    }
    calcProperties() {
        ...
        const r = this.r;
        const R = __classPrivateFieldGet(this, _Reuleaux_R, "f");
        __classPrivateFieldSet(this, _Reuleaux_h, root3 * r * cospi6 - Math.sqrt(Math.pow(root3 * r - R, 2) - 3 * r * r * sinpi6 * sinpi6), "f");
        const l = Math.sqrt(3 * r * r + __classPrivateFieldGet(this, _Reuleaux_h, "f") * __classPrivateFieldGet(this, _Reuleaux_h, "f") - 2 * root3 * r * __classPrivateFieldGet(this, _Reuleaux_h, "f") * cospi6);
        __classPrivateFieldSet(this, _Reuleaux_phi, Math.acos((3 * r * r + l * l - __classPrivateFieldGet(this, _Reuleaux_h, "f") * __classPrivateFieldGet(this, _Reuleaux_h, "f")) / (2 * root3 * r * l)) + Math.PI / 6, "f");
    }
    parametric(s) {
        const root3 = Math.sqrt(3);
        const sideLength = root3 * this.r * (2 * Math.PI / 3 - 2 * __classPrivateFieldGet(this, _Reuleaux_phi, "f"));
        const cornerLength = __classPrivateFieldGet(this, _Reuleaux_R, "f") * 2 * __classPrivateFieldGet(this, _Reuleaux_phi, "f");
        // For sides, 0 <= t <= 2pi/3 -2phi
        const side = t => new Vector([root3 * this.r * (Math.sin(t + __classPrivateFieldGet(this, _Reuleaux_phi, "f")) - 0.5), this.r * (root3 * Math.cos(t + __classPrivateFieldGet(this, _Reuleaux_phi, "f")) - 0.5)]);
        // For corners, 0 <= t <= 2phi
        const corner = t => new Vector([__classPrivateFieldGet(this, _Reuleaux_R, "f") * Math.sin(t - __classPrivateFieldGet(this, _Reuleaux_phi, "f")), __classPrivateFieldGet(this, _Reuleaux_R, "f") * Math.cos(t - __classPrivateFieldGet(this, _Reuleaux_phi, "f")) + this.r - __classPrivateFieldGet(this, _Reuleaux_h, "f")]);
        const perim = this.perimeter;
        ...
        if (s < sideLength) { // 1st side
            ...
            const t = (s - sideLength) / __classPrivateFieldGet(this, _Reuleaux_R, "f");
            ...
            const t = (s - 2 * sideLength - cornerLength) / __classPrivateFieldGet(this, _Reuleaux_R, "f");
            ...
            const t = (s - 3 * sideLength - 2 * cornerLength) / __classPrivateFieldGet(this, _Reuleaux_R, "f");
            return corner(t);
        }
    }
    penPoints() {
    ...
    penPoints() {
        const spacingR = 0.25 / __classPrivateFieldGet(this, _Reuleaux_r, "f");
...
_Reuleaux_r = new WeakMap(), _Reuleaux_R = new WeakMap(), _Reuleaux_h = new WeakMap(), _Reuleaux_phi = new WeakMap();
Reuleaux.shape = "reuleaux";
class Line {
    ...
    get heading() {
        if (!this.lastPoint || !this.previousPoint)
            return null;
...
class Tutorial {
    constructor() {
        _Tutorial_instances.add(this);
        _Tutorial_hint.set(this, void 0);
        _Tutorial_stepCompleteCallback.set(this, this.nextStep.bind(this));
        this.currentStep = -1;
        this.steps = [];
        __classPrivateFieldSet(this, _Tutorial_hint, document.createElement("p"), "f");
        __classPrivateFieldGet(this, _Tutorial_hint, "f").classList.add("hint");
        window.addEventListener("wheel", __classPrivateFieldGet(this, _Tutorial_instances, "m", _Tutorial_repositionHint).bind(this));
        window.addEventListener("touchmove", __classPrivateFieldGet(this, _Tutorial_instances, "m", _Tutorial_repositionHint).bind(this));
        window.addEventListener("resize", __classPrivateFieldGet(this, _Tutorial_instances, "m", _Tutorial_repositionHint).bind(this));
    }
    ...
    nextStep() {
        ...
        __classPrivateFieldGet(this, _Tutorial_instances, "m", _Tutorial_unloadCurrentStep).call(this);
        this.currentStep++;
        __classPrivateFieldGet(this, _Tutorial_instances, "m", _Tutorial_loadCurrentStep).call(this);
        if (this.finished)
            console.log("Tutorial finished");
        return this;
    }
}
_Tutorial_hint = new WeakMap(), _Tutorial_stepCompleteCallback = new WeakMap(), _Tutorial_instances = new WeakSet(), _Tutorial_repositionHint = function _Tutorial_repositionHint() {
    if (!this.started || this.finished)
        return;
    const element = this.steps[this.currentStep].element;
    const hint = __classPrivateFieldGet(this, _Tutorial_hint, "f");
    ...
    hint.style.top = `${y}px`;
}, _Tutorial_loadCurrentStep = function _Tutorial_loadCurrentStep() {
    if (!this.started || this.finished)
        return;
    const step = this.steps[this.currentStep];
    step.element.scrollIntoView({ behaviour: "smooth", block: "nearest" });
    const parent = step.element.offsetParent;
    __classPrivateFieldGet(this, _Tutorial_hint, "f").classList.add(parent.offsetLeft > window.innerWidth / 2 ? "left" : "right");
    __classPrivateFieldGet(this, _Tutorial_hint, "f").textContent = step.text;
    parent.appendChild(__classPrivateFieldGet(this, _Tutorial_hint, "f"));
    step.element.classList.add("hint-element");
    step.events.forEach(ev => step.element.addEventListener(ev, __classPrivateFieldGet(this, _Tutorial_stepCompleteCallback, "f")));
    __classPrivateFieldGet(this, _Tutorial_instances, "m", _Tutorial_repositionHint).call(this);
}, _Tutorial_unloadCurrentStep = function _Tutorial_unloadCurrentStep() {
    if (!this.started || this.finished)
        return;
    const step = this.steps[this.currentStep];
    __classPrivateFieldGet(this, _Tutorial_hint, "f").classList.remove("left");
    __classPrivateFieldGet(this, _Tutorial_hint, "f").classList.remove("right");
    __classPrivateFieldGet(this, _Tutorial_hint, "f").parentElement.removeChild(__classPrivateFieldGet(this, _Tutorial_hint, "f"));
    step.element.classList.remove("hint-element");
    step.events.forEach(ev => step.element.removeEventListener(ev, __classPrivateFieldGet(this, _Tutorial_stepCompleteCallback, "f")));
};
...
window.addEventListener("load", function () {
    ...
    const createWheels = function () {
        ...
        if (controls.guideType.getValue() !== Circle)
            args.push(controls.guideParam.getValue() / 100);
            ...
            if (!drawing.currentLine.started) {
                ...
                while (sLast < s) {
                    ...
                    for (let i = 0; i <= 10; i++) {
                        ...
                        while (headingLast - heading > Math.PI)
                            headingLast -= 2 * Math.PI;
                        while (heading - headingLast > Math.PI)
                            headingLast += 2 * Math.PI;
                        // If the angle covered by the segment is sufficiently small, we're done
                        if (Math.abs(headingLast - heading) <= maxAngle)
                            break;
