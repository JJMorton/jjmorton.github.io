...
function assemble(lines, onerror) {
    ...
    const getTransition = function (symbol) {
        if (/^(INP|OUT|HLT)$/.test(symbol))
            return transitions.INSTR;
        if (/^(ADD|SUB|STA|LDA|BRA|BRZ|BRP)$/.test(symbol))
            return transitions.INSTR_ADDR;
        if (/^DAT$/.test(symbol))
            return transitions.INSTR_DATA;
        if (/^[A-Z|a-z][A-Z|a-z|0-9]*$/.test(symbol))
            return transitions.LABEL;
        if (/^[0-9]{1,2}$/.test(symbol))
            return transitions.ADDR;
        if (/^[0-9]{1,3}$/.test(symbol))
...
function parseMachineCode(lines, onerror) {
    ...
    if (valid)
        console.info("[MACHINE CODE] Success");
...
function setValidateMessage(textbox, message) {
    ...
    if (!textbox.classList.contains("validate"))
        return;
    ...
    const statemachine = new (function () {
        ...
        const transitions = {
            ...
            [states.IDLE]: {
                ...
                reset: () => {
                    for (let addr = 0; addr < computer.memory.length; addr++)
                        computer.setMemory(addr, 0);
            ...
            [states.WAITING]: {
                input: () => {
                    ...
                    if (!textboxes.input.checkValidity())
                        return;
        ...
        this.dispatch = function (actionName) {
            if (!Object.values(states).includes(state))
                return console.error("[STATE] Invalid state");
